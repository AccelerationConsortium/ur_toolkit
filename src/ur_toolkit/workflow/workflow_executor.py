...
class WorkflowExecutor:
    ...
    def __init__(self, taught_positions_file=None):
        ...
        if taught_positions_file is None:
            taught_positions_file = (
                Path(__file__).parent.parent / "positions" / "taught_positions.yaml"
            )
        ...
        print(
            f"📍 Loaded {len(self.taught_positions.get('positions', {}))} taught positions"
        )
    ...
    def init_visual_servo_engine(self):
        ...
        try:
            ...
            self.visual_servo_engine = VisualServoEngine(
                None, self.positions_file, detector
            )
    ...
    def execute_workflow_dict(self, workflow, step_mode=False):
        ...
        if step_mode:
            print(
                "🐾 Step-by-step mode: Press ENTER to continue each step, 'q' to quit"
            )
        print("=" * 60)
        ...
        for i, step in enumerate(steps, 1):
            ...
            if step_mode:
                ...
                try:
                    user_input = (
                        input("Continue? (ENTER/q to quit/s to skip): ").lower().strip()
                    )
    ...
    def _generate_step_name(self, step, step_number):
        ...
        if action in ["movel", "movej"]:
            ...
            return (
                f"Offset move ({', '.join(offset_str) if offset_str else 'no offset'})"
            )
        elif action == "gripper_open":
            return "Open gripper"
        elif action == "gripper_close":
    ...
    def _move_to_position(self, step):
        ...
        if use_visual_servo and self.visual_servo_engine:
            ...
            try:
                corrected_pose = self.visual_servo_engine.get_corrected_pose(
                    position_name, coordinates
                )
    ...
    def _visual_servo_action(self, step):
        ...
        try:
            # Global master enable check
            if (
                hasattr(self.visual_servo_engine, "config")
                and not self.visual_servo_engine.config.enabled
            ):
                print(
                    f"⚪ Visual servo globally disabled (visual_servo.enabled=False) - skipping correction for '{position_name}'"
                )
                return True  # Treat as successful no-op
            # Execute visual servoing (Option B will update positions file)
            # Per-step override: disable_rotation=True forces rotation corrections off
            if step.get("disable_rotation"):
                original_flag = getattr(
                    self.visual_servo_engine.config, "enable_rotation", True
                )
                # Temporarily disable via config object attribute (non-persistent if config reloads)
                try:
                    self.visual_servo_engine.config.config[
                        "visual_servo.enable_rotation"
                    ] = False
                except Exception:
                    pass
            else:
            ...
            success, result_data = self.visual_servo_engine.visual_servo_to_position(
                position_name, update_stored_pose=True
            )
            # Restore rotation flag if we modified it
            ...
            if success:
                ...
                # Reload positions so subsequent move steps use updated propagated coordinates
                try:
                    reloaded = self._load_taught_positions(self.positions_file)
                    if reloaded:
                        self.taught_positions = reloaded
                        print(
                            "🔄 Reloaded taught positions after visual servo propagation"
                        )
                    else:
                        print("⚠️  Failed to reload taught positions after servo")
                except Exception as e:
                    print(f"⚠️  Error reloading positions after servo: {e}")
                return True
            else:
                # Graceful failure handling: allow workflow to continue or abort based on step flag
                abort_on_fail = step.get("abort_on_fail", True)
                print("❌ Visual servo failed")
                if abort_on_fail:
                    print(
                        "⏹️  Aborting workflow due to visual servo failure (abort_on_fail=True)"
                    )
                    return False
                else:
                    print(
                        "➡️  Continuing despite visual servo failure (abort_on_fail=False)"
                    )
                    return True

        except Exception as e:
            abort_on_exception = step.get("abort_on_exception", True)
            print(f"❌ Visual servo error: {e}")
            if abort_on_exception:
                print("⏹️  Aborting workflow due to exception (abort_on_exception=True)")
                return False
            else:
                print("➡️  Continuing despite exception (abort_on_exception=False)")
                return True
...
def main():
    ...
    parser.add_argument(
        "--list-positions", action="store_true", help="List available positions"
    )
