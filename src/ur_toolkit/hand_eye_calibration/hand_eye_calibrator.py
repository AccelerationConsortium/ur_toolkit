...
class HandEyeCalibrator:
        ...
        try:
            ...
            for tag_id in self.tag_ids:
                if tag_id in detections:
                    ...
                    if det['confidence'] > 0.8:  # High confidence threshold
                        valid_detections[tag_id] = det
                    else:
                        print(f"⚠️  Low confidence detection for tag {tag_id}: {det['confidence']:.3f}")
    ...
    def _tag_detection_to_transform(self, detection: Dict) -> np.ndarray:
        ...
        tvec = np.array(detection['tvec']).flatten()
        rvec = np.array(detection['rvec']).flatten()
    ...
    def _print_calibration_results(self):
        ...
        print(f"Transform Matrix (Camera to End-Effector):")
