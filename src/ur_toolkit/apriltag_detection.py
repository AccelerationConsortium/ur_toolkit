...
class AprilTagDetector:
    ...
    def __init__(self, tag_family=None, tag_size=None, calibration_file=None):
        ...
        print(f"🏷️  AprilTag Detector: {self.tag_family}, " f"size={self.tag_size}m")
    ...
    def load_calibration(self, calibration_file):
        ...
        try:
            ...
            print(
                f"   Focal length: fx={self.camera_matrix[0, 0]:.1f}, fy={self.camera_matrix[1, 1]:.1f}"
            )
    ...
    def detect_tags(self, image, estimate_pose=True):
        ...
        for tag in tags:
            ...
            if (
                estimate_pose and self.pose_estimation_enabled and tag.hamming == 0
            ):  # Only use perfect detections for pose
    ...
    def draw_pose_axes(self, image, pose, axis_length=30):
        ...
        projected, _ = cv2.projectPoints(
            axes_points, rvec, tvec, self.camera_matrix, self.dist_coeffs
        )

        projected = projected.reshape(-1, 2)
        if not np.isfinite(projected).all() or projected.shape[0] < 4:
            return

        # Robust point converter ensuring built-in Python ints
        def _to_pt(row):
            x = row[0]
            y = row[1]
        ...
        try:
            origin = _to_pt(projected[0])
            x_pt = _to_pt(projected[1])
            y_pt = _to_pt(projected[2])
            z_pt = _to_pt(projected[3])

            # Draw simple axes; if it fails, silently skip (detection already succeeded)
            cv2.line(image, origin, x_pt, (0, 0, 255), 2)  # X-axis
            cv2.line(image, origin, y_pt, (0, 255, 0), 2)  # Y-axis
            cv2.line(image, origin, z_pt, (255, 0, 0), 2)  # Z-axis
        except Exception:
            # Skip axes drawing to avoid crashing the script
            return
...
def main():
    ...
    parser.add_argument(
        "--port", type=int, help="Camera server port (overrides config)"
    )
    parser.add_argument(
        "--calibration", help="Camera calibration file (overrides config)"
    )
    parser.add_argument(
        "--tag-family",
        choices=[
    ...
    parser.add_argument(
        "--tag-size", type=float, help="AprilTag size in millimeters (overrides config)"
    )
    parser.add_argument(
        "--save-detections", action="store_true", help="Save images with detections"
    )
    parser.add_argument(
        "--continuous", action="store_true", help="Continuous detection mode"
    )
    parser.add_argument(
        "--auto",
        action="store_true",
        help="Non-interactive single capture (skip ENTER prompt)",
    )
    ...
    if args.host or args.port:
        ...
        # PiCamConfig does not currently accept a timeout override parameter; it uses an internal default.
        # We intentionally avoid passing unsupported kwargs.
    ...
    if args.continuous:
            ...
            while True:
                ...
                if detections:
                    ...
                    for det in detections:
                        ...
                        print(
                            f"   ID {det['tag_id']}: quality={det['decision_margin']:.2f}{pose_info}"
                        )
                else:
        ...
        if not args.auto:
            input("Press ENTER to capture and detect (use --auto to skip)...")
        ...
        if detections:
            ...
            for i, det in enumerate(detections):
                ...
                print(
                    f"   Hamming Distance: {det['hamming']} (0=perfect, 1-2=good, >2=poor)"
                )
                ...
                if det["pose"]:
                    ...
                    print(
                        "   ❌ No pose estimation (missing calibration or poor detection)"
                    )
        else:
